...
<body>
								...
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										...
										<select id="menpiaoleixingSelect" name="menpiaoleixing" class="form-control">
												<option value=""></option>
												<option class="menpiaoleixingOption" value="成人票">
													成人票
												</option>
												<option class="menpiaoleixingOption" value="儿童票">
													儿童票
												</option>
										</select>	
									</div>	
									...
									<div class="form-group col-md-6">
										...
										<input name="jingdiantupian" id="jingdiantupian-input" type="hidden"></div>
											...
											<div class="form-group  col-md-12" >
												<label>景点攻略</label>
												<div><div class="upload"><input id="jingdiangonglveupload" name="file" type="file">选择文件</div>
												<script id="jingdiangonglveEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var jingdiangonglveUe = UE.getEditor('jingdiangonglveEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="jingdiangonglve" id="jingdiangonglve-input">
												</div>
											</div>	
	...
	<script>
		...
		// 文件上传
		function upload(){
			$('#jingdiangonglveupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('jingdiangonglveEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.jingdiangonglve != null && ruleForm.jingdiangonglve != 'null' && ruleForm.jingdiangonglve != ''){
				var jingdiangonglveMes = '' + ruleForm.jingdiangonglve;
				var jingdiangonglveUeditor = UE.getEditor('jingdiangonglveEditor');
				jingdiangonglveUeditor.ready(function() {
					jingdiangonglveUeditor.setContent(jingdiangonglveMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('jingdiangonglveEditor').hasContents()){
				$('#jingdiangonglve-input').attr('value',UE.getEditor('jingdiangonglveEditor').getContent());
			}
		}
		...
		function dataBind(){
			...
			$("#jingdiangonglve").val(ruleForm.jingdiangonglve);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.jingdiangonglve != null && res.data.jingdiangonglve != ''){

						$("#jingdiangonglve").val(res.data.jingdiangonglve);
                        var jingdiangonglveMes = '' + res.data.jingdiangonglve;
                        var jingdiangonglveUeditor = UE.getEditor('jingdiangonglveEditor');
                        jingdiangonglveUeditor.ready(function() {
                            jingdiangonglveUeditor.setContent(jingdiangonglveMes);
                        });
